...
def get_prompter(render, log, confirm=True):
    ...
    def prompt(
        ...
        def read_input():
            ...
            user_message = render(context, 'prompt.j2', out=None).strip()
        ...
        while True:
            try:
                ...
                log.info("got user input `{user_input}`".format(user_input=user_input))

                # Map
                user_input = map_input(user_input)
                log.info("user input converted to `{user_input}` (by input map)".format(
                    user_input=user_input, map_input=map_input))

                # Validate
                user_input = validate_input(user_input)
                log.info("user input converted to `{user_input}` (by input validation)".format(
